...
if st.button("Predict"):
    ...
    reliable_probability = probabilities[0][0] # Probability of class 0 (reliable)
    risky_probability = probabilities[0][1] # Probability of class 1 (risky)
    
    prediction = model.predict(preprocessed_data)
    
    st.write(f"Probability of being **reliable** (approved): {reliable_probability:.2f}")
    st.write(f"Probability of being **risky** (not approved): {risky_probability:.2f}")

    # Show whether the applicant is likely to be approved or not
    if prediction == 1:  # Class 1 ("risky", not approved)
        st.write(f"The applicant is **unlikely** to be approved for a credit card with a probability of {risky_probability:.2f}.")
    else:  # Class 0 ("reliable", approved)
        st.write(f"The applicant is **likely** to be approved for a credit card with a probability of {reliable_probability:.2f}.")
